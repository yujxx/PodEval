...
// new add by cxy
WaveformVisualizer.prototype.updateWaveform = function(stimulus) {
  var newAudioBuffer = stimulus.getAudioBuffer();
  if (newAudioBuffer) {
    var numberOfChannels = newAudioBuffer.numberOfChannels;
    this.avgOriginalSamples = new Float32Array(newAudioBuffer.length);
    for (var i = 0; i < newAudioBuffer.length; ++i) {
      var sum = 0;
      for (var j = 0; j < numberOfChannels; j++) {
        sum += newAudioBuffer.getChannelData(j)[i];
      }
      var avg = sum * (1.0 / numberOfChannels);
      this.avgOriginalSamples[i] = avg;
    }
