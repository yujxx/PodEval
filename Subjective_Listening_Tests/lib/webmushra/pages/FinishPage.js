...
FinishPage.prototype.storeParticipantData = function() {
  ...
  for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      if(element.type==="likert2"){
        var multi_result = [];
        var checkboxes = $("input[name='"+element.name + "__response']:checked");
        checkboxes.each(function() {
          var checkbox_value = $(this).val();
          if (checkbox_value.toLowerCase() === "other"){
            checkbox_value = $("input[id='other']").val();
          }
          multi_result.push(checkbox_value);
        });
        this.session.participant.response[this.session.participant.response.length] = multi_result.join("/");
...
FinishPage.prototype.render = function (_parent) {
    ...
    for (i = 0; i < this.pageConfig.questionnaire.length; ++i) {
      ...
      } else if (element.type === "likert2"){
        this.likert = new LikertScale2(element.response, element.name + "_");
        var td = $("<td></td>");
        table.append($("<tr></tr>").append(
          $("<td><strong>"+ element.label +"</strong></td>"),
          td
