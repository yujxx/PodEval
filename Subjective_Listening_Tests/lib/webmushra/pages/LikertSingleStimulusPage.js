...
LikertSingleStimulusPage.prototype.load = function () {  
  ...
  if(this.results.length > 0){
    for (var i = 0; i < this.likerts.length; ++i) {
      if(this.likerts[i].responseConfig[0].type=="option"){
        // resume option type text input box content
        $("#"+this.likerts[i].prefix+"_comment_option").val(this.results[i]);
      } else if(this.likerts[i].responseConfig[0].type=="text"){
        // resume text type text input box content
        $("#"+this.likerts[i].prefix+"_text_input").val(this.results[i]);
      } else {
...
LikertSingleStimulusPage.prototype.save = function () {
  ...
  for (var i = 0; i < this.likerts.length; ++i) {
    ...
    if(this.likerts[i].responseConfig[0].type=="option"){
      this.results[i] = $("#"+this.likerts[i].prefix+"_comment_option").val();
      }else if(this.likerts[i].responseConfig[0].type=="text"){
      this.results[i] = $("#"+this.likerts[i].prefix+"_text_input").val();
    }
  }
