...
// Add: generate filename as date-time-Username
date_default_timezone_set('Asia/Shanghai');
$dateTimeStr = date('Y-m-d-H-i-s');
$username = isset($session->participant->response[0]) ? sanitize($session->participant->response[0], true) : 'unknown';
$customFilename = $dateTimeStr . '-' . $username . $filepathPostfix;
...
if ($write_mushra) {
	$filename = $filepathPrefix.$customFilename;
